...
- **성공 지표**: 2만 건의 수동 라벨링 데이터 기반 **validation accuracy 0.8>=** 달성 → 모델 신뢰성 확보
...
이 프로젝트에서는 초기 데이터 약 **108,000건**에서 불필요/중복 데이터를 제거하여 **97,000건**으로 정제하였습니다.

이후 전체 데이터에서 **10% 무작위 샘플**을 추출하여 0은 부정 1은 긍적으로 직접 긍정/부정을 라벨링했습니다.  
- 샘플 내 **긍정/부정 비율**: 3.5 : 7.5  
- 추가적으로, 백신(`vaccine`), 코로나(`covid`), 부작용(`side effect`) 관련 언급 여부를 확인하여 True/False로 라벨링  
  - 결과: `False = 84%`, `True = 16%`  

이 데이터를 기반으로 **KoELECTRA 모델**을 활용한 머신러닝 예측을 수행하여, 전체 데이터에 대한 감정 분포와 관련 여부를 추정하였습니다.
...
분석: 전체 데이터에서 부정적 의견이 약 **76%**로 다수를 차지하며, 긍정은 약 24%로 나타났습니다.
...
분석: 전체 데이터 중 백신/코로나/부작용 관련 언급이 있는 글은 약 **16.7%**에 불과합니다.
...
모델 예측 결과를 기반으로, **시간의 흐름에 따라 부정 감성 비율**이 어떻게 변화하는지 시각화하였습니다.  
...
- 그러나 본 분석 그래프에서는 수치상 **관련성이 거의 없는 것으로 나타남**  
...
시도	설명	추천 값
Learning rate 조정	더 세밀한 학습 시	1e-5 또는 3e-5
...
Validating: 100%|███████████████████████████████| 27/27 [00:02<00:00, 12.41it/s]
Training:   0%|                                         | 0/105 [00:00<?, ?it/s]Epoch 1 | Train Loss: 0.4607 | Val Acc: 0.8449

===== Epoch 2 / 3 =====
Training: 100%|███████████████████████████████| 105/105 [00:24<00:00,  4.28it/s]
Validating: 100%|███████████████████████████████| 27/27 [00:02<00:00, 12.42it/s]
Epoch 2 | Train Loss: 0.4438 | Val Acc: 0.8449

===== Epoch 3 / 3 =====
Training: 100%|███████████████████████████████| 105/105 [00:24<00:00,  4.24it/s]
Validating: 100%|███████████████████████████████| 27/27 [00:02<00:00, 12.69it/s]
Epoch 3 | Train Loss: 0.4277 | Val Acc: 0.8449

 Validation Accuracy (검증 정확도) 고착 현상가장 눈에 띄는 점은 검증 정확도(Val Acc)가 3 에포크 내내 $0.8449$로 전혀 변하지 않았다는 것입니다.Positive (긍정적 신호): 에포크 1부터 $84.49%$라는 비교적 높은 초기 정확도를 달성했습니다.Negative (부정적 신호): 에포크가 진행되면서 훈련 손실(Train Loss)은 $0.4607 \to 0.4438 \to 0.4277$로 꾸준히 감소하고 있는데도, 이 개선된 학습 효과가 검증 데이터에는 전혀 반영되지 않았습니다. 이는 모델이 훈련 데이터에만 적합되거나 (약한 과적합), 혹은 데이터나 설정에 문제가 있을 가능성을 시사합니다.🟡 학습 진행 상황훈련 손실 감소: 훈련 손실은 3 에포크 동안 꾸준히 감소했으므로, 학습 과정 자체는 정상적으로 진행되었습니다. 모델은 훈련 데이터에 대해서는 계속해서 더 잘 분류하도록 발전하고 있습니다.🧐 문제 원인 분석 및 다음 단계검증 정확도가 고정되는 현상은 대개 다음 중 하나 이상의 이유로 발생합니다.1. 데이터 불균형 또는 특이성 문제데이터셋의 클래스 불균형 (Class Imbalance):두 클래스(sentiment 0과 1)의 비율이 극도로 불균형할 경우, 모델은 다수 클래스만 예측해도 높은 정확도를 얻을 수 있습니다. 예를 들어, 데이터의 $84.49%$가 한쪽 클래스라면, 모델이 무조건 그 클래스만 예측해도 정확도는 $84.49%$가 되며, 이 값에서 더 이상 개선되기 어려울 수 있습니다.✅ 해결책: 훈련 데이터(train_labels)와 검증 데이터(val_labels)의 클래스 분포를 확인해 보세요. (예: pd.Series(val_labels).value_counts(normalize=True))검증 데이터셋의 고유한 특성 부족:검증 데이터셋의 개수가 작거나, 포함된 텍스트가 훈련 데이터와 너무 유사하거나, 혹은 너무 쉬운 경우 모델이 에포크 1에서 이미 최적의 성능을 달성했을 수 있습니다.2. 하이퍼파라미터 문제낮은 학습률 (Learning Rate):lr=5e-5는 BERT/Electra 파인튜닝에 표준적인 값입니다. 그러나 데이터셋이 작다면, 이 학습률로는 모델이 더 깊이 최적화되지 못하고 초기 성능에 갇힐 수 있습니다.✅ 해결책: 학습률을 약간 더 높여보는 것을 고려해 볼 수 있습니다 (예: $1e-4$ 또는 $2e-5$와 같이 조금 더 공격적이거나 보수적인 값으로 실험).

 이후 클래스 가중치를 적용한 수정 코드 main 함수와 train_epoch 함수에 클래스 가중치 계산 및 적용 로직을 추가했습니다.

 주요 변경 사항 요약클래스 가중치 계산 및 적용 (불균형 해결):train_labels를 분석하여 각 클래스의 빈도수를 구했습니다.class_weights를 계산하여 수가 적은 클래스에 높은 가중치를 부여했습니다.train_epoch과 eval_epoch 함수에서 torch.nn.CrossEntropyLoss를 사용할 때 이 가중치를 적용하도록 코드를 수정했습니다. (정확도 $0.8449$가 데이터 불균형 비율이라면, 이 수정이 가장 중요합니다.)학습률(Learning Rate) 상향:AdamW의 lr을 표준인 $5e-5$에서 $1e-4$로 높였습니다. 이는 고착된 성능($0.8449$)에서 벗어나 모델이 더 빠르게 최적화되도록 시도하는 방법입니다.에포크 횟수 증가:에포크를 3회에서 5회로 늘렸습니다. 훈련 손실은 계속 감소했기 때문에, 5회까지 관찰하여 정확도가 상승하는 임계점을 찾을 수 있습니다.검증 손실 (Val Loss) 출력:eval_epoch 함수가 검증 정확도 외에 검증 손실도 반환하도록 수정했습니다. 손실 값을 함께 보면 모델의 과적합 여부를 더 명확하게 판단할 수 있습니다.

 파인튜닝 결과 재분석: 심각한 불안정성 발견수정된 코드로 5 에포크를 실행한 결과, 이전보다 더 심각한 문제가 발생했습니다. 학습률($1e-4$) 상향과 클래스 가중치 적용에도 불구하고 모델의 성능이 매우 불안정하며, 학습이 제대로 진행되지 않고 있습니다.1. 결과 요약 및 문제점EpochTrain LossVal AccVal Loss1$0.7012$$0.1551$$0.7061$2$0.6974$$0.8449$$0.6878$3$0.6880$$0.1551$$0.7116$4$0.6938$$0.8449$$0.6876$5$0.6923$$0.8449$$0.6880$⚠️ 핵심 문제: 정확도 급변 및 반복 (Oscillation)검증 정확도(Val Acc)가 에포크마다 **$0.1551 \to 0.8449 \to 0.1551 \to 0.8449$**로 극단적으로 진동하고 있습니다.Val Acc $0.8449$: 이는 이전 분석에서 지적했듯이, **다수 클래스(Class 0)**만 예측했을 때 얻을 수 있는 정확도($1399 / (1399+275) \approx 0.8359$이지만 검증 데이터셋의 비율은 $0.8449$일 수 있음)와 매우 가깝습니다. 즉, 모델이 Class 0만 예측하고 있다는 뜻입니다.Val Acc $0.1551$: 이는 **소수 클래스(Class 1)**만 예측했을 때 얻을 수 있는 정확도와 거의 같습니다. $(275 / (1399+275) \approx 0.1641$와 가깝습니다). 즉, 모델이 Class 1만 예측하고 있다는 뜻입니다.모델이 클래스 가중치가 적용된 상태에서 학습률이 높다 보니, 에포크마다 극단적인 클래스 예측을 반복하며 불안정한 상태에 빠져 있습니다. 클래스 가중치와 높은 학습률이 상호작용하여 학습을 불안정하게 만들었습니다.2. 🎯 권장 수정 방안 (가장 안정적인 설정)이 문제를 해결하기 위해, 원래의 안정적인 하이퍼파라미터 설정으로 되돌리고 동시에 클래스 가중치 적용을 유지하여 모델이 두 클래스를 모두 예측하도록 유도하는 것이 최선입니다.수정 원칙:학습률(LR)을 표준적인 값으로 낮춥니다: 불안정성의 가장 큰 원인입니다.클래스 가중치는 유지합니다: 불균형 문제를 해결하기 위함입니다.에포크는 5회로 유지합니다: 개선 여부를 관찰합니다.

 최종 파인튜닝 결과 분석: 학습 정체 문제제공해주신 최종 5 에포크 결과를 분석하면, 클래스 가중치와 안정적인 학습률(5e-5)을 적용했음에도 불구하고 모델의 성능이 개선되지 않고 정체되어 있습니다.1. 결과 요약 및 해석EpochTrain LossVal AccVal Loss주요 변화1$0.6961$$0.8449$$0.6876$초기 성능2$0.6961$$0.8449$$0.6872$성능 변화 없음3$0.6921$$0.8401$$0.6904$잠시 하락 (노이즈)4$0.6940$$0.8449$$0.6863$복귀 및 정체5$0.6899$$0.8449$$0.6867$성능 고착Val Acc $0.8449$ 고착: 이 값은 전체 데이터셋의 약 $84%$가 다수 클래스(Class 0, 긍정/부정 중 하나)에 속한다는 의미일 가능성이 매우 높습니다.Train Loss/Val Loss: 손실 값들은 $\approx 0.69$ 근처에서 거의 변동이 없거나 미미하게 감소하고 있습니다. 이는 모델이 학습에 필요한 신호를 거의 찾지 못하고 있다는 심각한 증거입니다.클래스 가중치 무력화: 클래스 가중치를 적용했음에도 불구하고, 모델은 학습률($5e-5$)이 낮고 사전 학습된 가중치가 워낙 강하여 여전히 다수 클래스만 예측하는 전략을 고수하고 있습니다.2. 🔍 학습 정체의 가장 유력한 원인1. 데이터 문제 (가장 유력):라벨 노이즈: CSV 파일의 레이블링(sentiment) 자체에 오류나 모호함이 많아, 모델이 긍정과 부정을 명확히 구분할 수 없는 상태입니다.텍스트-레이블 불일치: 텍스트(text)와 레이블(sentiment) 간의 상관관계가 약해서, KoELECTRA 같은 강력한 모델조차 패턴을 찾지 못하는 것입니다.2. 모델 설정 문제 (두 번째 유력):매우 낮은 학습률: $5e-5$는 안정적이지만, 가중치가 고착된 상태에서는 모델이 갇힌 지역 최저점에서 빠져나오지 못할 수 있습니다.3. 🚀 최종 권장 방안: 학습률 재조정 및 데이터 확인성능 개선을 위해 하이퍼파라미터를 다시 한 번 조정하고, 동시에 데이터셋 자체를 점검해 보는 것이 필요합니다.1. 하이퍼파라미터 조정: 학습률 중간값으로 재설정이전의 극심한 불안정성($1e-4$)과 현재의 정체($5e-5$) 사이의 중간값을 사용하여 모델이 정체된 최저점에서 벗어나도록 시도해야 합니다.목표: **$2e-5$**로 학습률을 낮춰서, 클래스 가중치와 함께 소수 클래스를 예측하도록 유도합니다.

 학습 결과 분석 및 다음 단계학습률을 $\text{lr}=2e-5$로 재조정한 결과, 모델은 이전의 고착 상태($0.8449$ 고정)에서 벗어났습니다. 이제 모델은 소수 클래스를 예측하기 시작했지만, 검증 성능이 매우 불안정하게 진동하는 새로운 문제를 보이고 있습니다.1. 📊 학습 결과 요약EpochTrain LossVal AccVal Loss주요 특징1$0.6940$$0.8449$$0.6826$다수 클래스 예측 (초기 상태)2$0.6797$$0.5370$$0.6704$성능 급락 (소수 클래스 예측 시도)3$0.6407$$0.6945$$0.6752$회복 시도4$0.5705$$0.6277$$0.6861$성능 하락5$0.4925$$0.7446$$0.7324$Val Loss 급증 (과적합 징후)2. 🧐 주요 관찰 및 분석⚠️ 훈련 손실 vs. 검증 손실 (과적합 징후)Train Loss: $0.6940 \to \dots \to **0.4925$**로 꾸준히, 그리고 가파르게 감소했습니다. 이는 모델이 훈련 데이터에 대해서는 매우 잘 학습하고 있다는 뜻입니다.Val Loss: $0.6826$에서 시작해 $0.6704$로 최저점을 찍은 후, $0.6752 \to 0.6861 \to **0.7324$**로 크게 증가했습니다.$\text{Train Loss}$는 급락하는데 $\text{Val Loss}$가 증가하는 현상은 전형적인 과적합(Overfitting) 징후입니다. 모델이 훈련 데이터의 특성과 노이즈에 과하게 맞춰지기 시작했고, 일반화 능력이 떨어지고 있습니다.📉 검증 정확도 (Val Acc) 진동$\text{Val Acc}$가 $0.8449$에서 $0.5370$까지 떨어졌다가 다시 $0.7446$으로 회복하는 등 불안정하게 진동하고 있습니다. 이는 클래스 가중치가 적용되면서 모델이 다수 클래스($0.8449$)만 찍는 전략과 소수 클래스도 찍어보려는 전략 사이에서 혼란을 겪고 있기 때문입니다.최적의 성능은 Epoch 1의 $0.8449$나 Epoch 3의 $0.6945$ 주변에 있었을 가능성이 높습니다.3. 🛠️ 최종 권장 수정 및 다음 단계현재 상황은 **'과적합'**이 가장 큰 문제입니다. 모델이 소수 클래스를 예측하는 방법을 배우기 시작했지만, 훈련 데이터에 너무 빨리 적응해버렸습니다.1. 에포크 횟수 줄이기 (조기 종료 효과)과적합을 방지하는 가장 확실한 방법은 $\text{Val Loss}$가 최저점을 찍고 다시 증가하기 시작하는 시점에서 학습을 멈추는 것입니다.$\text{Val Loss}$는 Epoch 2에서 $0.6704$로 최저점을 찍었습니다.권장 수정: 에포크 횟수를 5회에서 3회로 줄여서, 과적합이 심해지기 전에 학습을 종료해야 합니다.2. 검증 지표 변경 (F1-Score 도입)정확도($\text{Val Acc}$)는 불균형 데이터셋에서는 오해의 소지가 있습니다. 소수 클래스 예측 능력을 확인하기 위해 F1-Score를 핵심 지표로 도입해야 합니다.
