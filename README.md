...
저희는 수많은 소스를 통합하고 양질의 데이터ㅕ를 확보하기 위해 다음 **4단계의 실제 정제 파이프라인**을 적용했습니다.
...
###  최종 라벨 데이터 정규화 (Label Mapping)

**목표:**  
$\text{BERT}$ 모델 학습 시 라벨 인식 오류를 방지하고 **Accuracy**를 확보합니다.

**처리:**  
수동 라벨링 파일 내의 `'Positive'` / `'Negative'` 라벨을  
$\text{BERT}$가 요구하는 **0과 1** (`0`: Negative, `1`: Positive)로 정확하게 **Mapping**하여 모델 학습에 투입했습니다.

---

### ✅ 결론

이 엄격한 **4단계 전처리 과정**을 통해 데이터 내 모든 **Garbage Noise**를 제거하고,  
**$\mathbf{BERT}$ 모델의 신뢰성 (Accuracy 82.04%)**을 보장할 수 있는  
**고품질 데이터셋**을 최종 구축했습니다.
