...
그동안 온라인 커뮤니티에 남겨진 댓글과 리뷰를 수집을 한 후 필요없는 데이터나 가비지 데이터를 정제한 후, 유의미한 정보만을 남기고 그것을 분석해 보는 것을 통해서 어떤 새로운 결론을 찾는 것을을 목표로 한다.

## 1-1. 연구 배경

- COVID-19 팬데믹 동안 사람들은 **언론 기사보다 온라인 커뮤니티 댓글, 포럼, SNS**에서 더 직접적인 감정과 의견을 드러냈음.
- 기존 연구(예: *COVID 공포지수 vs. 주가*)는 공포지수와 금융시장의 상관성은 잘 보여주지만 다음과 같은 질문에 충분히 답하지 못함:
  - “실제로 사람들은 **무엇 때문에** 불안해했는가?”
  - “논란의 중심은 *부작용*이었는가, 아니면 *정책과 의무화*였는가?”
...
1. **대규모 온라인 리뷰/댓글(초기 약 11만 건)**을 수집·정제하여  
   코로나/백신 관련 **고순도(high-purity) 텍스트 데이터셋**을 구축한다.
2. KoELECTRA, BERT, DeBERTa 등 다양한 딥러닝 기반 감성 분석 모델을 활용해  
   **백신 관련 여론의 긍·부정·중립 비율 및 시계열 변화를 정량화**한다.
3. 부정 감성 비율의 시계열과 **공포·탐욕 지수(Fear-Greed Index)** 등의 외부 지표를 비교하여  
   - 전처리의 적절성 검증  
   - 백신 논란의 핵심 원인이 **과학적 부작용인지**,  
     혹은 **정책·의무화·정치 갈등** 때문인지 분석한다.
4. LDA, BERTopic을 활용해 **토픽(주제) 단위로 논쟁의 축을 파악**하고,  
   - 부정 여론이 **의학·부작용 중심인지**,  
   - **정책/의무화·비용·정치 갈등** 중심인지 규명한다.

---

## 1-3. 연구 질문

**Q1.** 온라인 커뮤니티에서 **코로나 백신 관련 감성(긍·부정·중립)**의 전체 분포는 어떠한가?

**Q2.** 시간이 흐르면서 **부정 감성 비율은 어떻게 변해왔는가?**

**Q3.** 이 부정 감성 비율 시계열은  
**공포지수(또는 공포–탐욕 지수)**와 어느 정도 패턴을 공유하는가?

**Q4.** 토픽 모델링 결과, **부정/긍정 감성 각각에서 어떤 주제가 여론을 주도했는가?**  
예: *백신 부작용 / 마스크 의무화 / 의료비·병원 시스템 / 정치적 갈등* 등
...
```
**다음과 같이 키워드를 정한 이유*:
- 위와 같이 정의 한 이유는 앞서 실패했던 전처리 과정에서 실제로는 코로나와 관련이 있었던 댓글들 대부분이 포함하고 있었던 키워드들을 경험적으로 선정하였다.
---
...
- **향후 과제:**  
  -  모델 예측 편향을 줄이기 위해 **오버 샘플링**과 **클래스 가중치 적용** 같은 불균형 해소 기법 필수
...
여기부터는 코로나 백신 관련 댓글 데이터에 대한 KoElectra 기반 감성 분류 모델 개발 과정에서 발생한 핵심 문제점과 이를 해결하기 위해 적용된 고급 전략을 요약한다.
...
- **활용:** 이 개선된 결과를 기반으로 원본 데이터의 감성 트렌드 분석가능
...
- 이 수치는 특히 **‘긍정-중립’ 경계 모호성**을 반영하며,  
  그대로 사용 시 실제 해석에 혼란을 초래할 가능성이 있음

---

### 3. 🛠️ **Three-Class 모델 개선 방안**

51.63%의 정확도를 향상시키기 위해 다음 두 가지 개선 방안을 제시한다.
...
코로나 백신 논쟁의 핵심 축이 **의학적/과학적 우려 → 정책적/사회적 갈등**으로 전환되었다는 가설을 입증한다.
...
이는 **전처리가 성공적으로 이루어졌음을 강하게 증명**한다.

![공포-탐욕지수와 부정 감성 비율 비교](D.png)  
![공포-탐욕지수와 부정 감성 비율 비교 정규화](E.png)
...
##### 🔹 활용: “연구 결과의 활용 가능성” 요약
1. 공중보건 정책 및 위험 커뮤니케이션

본 연구는 온라인 여론에서 부정 감성이 백신 부작용 자체보다 ‘의무화·정책·경제·정치 갈등’에 더 강하게 반응한다는 점을 시계열·토픽 분석으로 보였다.
이는 향후 팬데믹 대응 시, 단순히 “백신은 안전하다”는 메시지보다 정책의 공정성, 강제성의 한계, 보상·비용 구조, 의사결정 과정의 투명성을 우선적으로 설명하는 커뮤니케이션 전략이 필요함을 시사한다.
또한 본 연구의 파이프라인을 기반으로, 실시간으로 온라인 부정 여론의 초점(부작용 vs 의무화 vs 정치)을 모니터링하는 “공중보건 여론 레이더” 시스템으로 확장 가능하다.

2. 의료기관·의료정책 분야 활용

부정 토픽에서 병원(hospital), 부채(debt), 비용(pay), 간호 인력(nurses) 등이 반복적으로 등장한 것은 백신 논란이 단순 의학적 문제를 넘어 의료 접근성과 비용에 대한 불만과 강하게 연결되어 있음을 보여준다.
의료기관 및 정책 담당자는 이 결과를 바탕으로, 백신·방역 정책 설계 시 의료비 부담 완화, 병원 시스템 과부하, 인력 문제를 함께 고려한 종합적 대책과 안내가 필요함을 확인할 수 있다.
특히 “무료 접종”, “정부 지원”, “보험 적용 범위” 등의 정보 제공을 강화하는 것이 부정 여론을 완화하는 중요한 수단이 될 수 있음을 시사한다.

3. 플랫폼·언론·팩트체크 분야 활용

토픽 모델링 결과, 코로나 ‘사기(hoax)’, ‘선전(propaganda)’, 반백신·음모론, 정치적 책임 공방과 같은 토픽이 뚜렷하게 분리되었다.
이는 플랫폼 사업자와 언론·팩트체크 기관이 어떤 키워드·프레이밍을 중심으로 왜곡 정보와 갈등 담론이 확산되는지를 정량적으로 파악하고,
특정 토픽(예: hoax/propaganda/anti-vaccine) 비율이 급증하는 시기에 팩트체크 콘텐츠 노출, 공신력 있는 정보 고정, 알고리즘 조정과 같은 개입 전략을 설계하는 근거로 활용될 수 있다.

4. 금융·리스크 관리 지표로서의 잠재력

정제된 댓글 기반 부정 감성 비율 시계열은 전통적인 공포·탐욕 지수와 유사한 패턴(DTW 기준)과 일정 수준의 음의 상관을 보였다.
이는 온라인 여론 데이터가 단순한 잡음이 아니라, **시장 심리·위험 인식의 보조 지표(proxy)**로 활용될 수 있음을 보여준다.
향후 특정 정책 이슈(백신 패스, 락다운 등) 관련 부정 여론 지표를 소비·이동·투자 심리와 연계한 리스크 모니터링 지표로 확장할 수 있는 가능성을 제시한다.

5. 학술 연구 및 시스템·서비스 확장 가능성

본 연구는 대규모 크롤링 → 다단계 전처리 → 주제 관련성 필터링 → 불균형 완화(클래스 가중치, 오버샘플링, Focal Loss) → DeBERTa 기반 감성 분석 → BERTopic 토픽 분석 → 시계열·외부 지표 결합까지 이어지는 통합 파이프라인을 제시한다.
이는 ▲방법론 중심 논문(텍스트 전처리·불균형 해소·모델 비교) ▲사회과학·보건학 논문(정책·신뢰·위험 커뮤니케이션 분석) 양쪽 모두에 활용 가능한 기반이 된다.
동일한 파이프라인을 활용하면, 코로나 백신뿐 아니라 기후 변화, 원전, AI 규제, 선거 이슈 등 다른 사회적 논쟁 주제로도 손쉽게 확장할 수 있어,
“이슈별 온라인 여론 모니터링 시스템” 또는 대시보드 서비스로 발전시킬 수 있는 잠재력이 크다.
